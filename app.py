...
with col1:
    ...
    nombre_paciente_audio = st.text_input(
        "Nombre del paciente (opcional, ayuda a encontrarlo luego en el Historial)"
    )
        ...
        if transcripciones:
            ...
            try:
                ...
                if nombre_paciente_limpio:
                    nombre_hist = f"{timestamp}_{nombre_limpio}_{nombre_paciente_limpio}.txt"
                else:
...
with st.expander(f"📁 Historial de Informes ({len(archivos_historial_todos)})", expanded=False):
    ...
    st.caption("La búsqueda también revisa el contenido de cada informe, no solo el nombre del archivo.")

    archivos_historial = sorted(archivos_historial_todos, reverse=True)
    if busqueda:
        termino = busqueda.lower()
        archivos_historial = [a for a in archivos_historial if _coincide_busqueda(a, termino)]
